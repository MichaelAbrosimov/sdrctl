...
- **Автор:** Codex
- **Статус:** Реализовано пакетом 3 — ожидает ревью Codex/Michael
...
- **Пакет 3 (безопасность/устойчивость: SDR-P1-04 + SDR-P2-02 + SDR-P2-03 +
  settling (в)) — реализован, ожидает ревью.** Secrets overlay
...
- критерий покоя (уточнён по пункту 5 повторного ревью Codex): состояние
  ...
  `core.DeviceQuiescent`, пункт (в) реализован пакетом 3 для выхода из
  карантина: `GateWrite` требует 3 подряд стабильных наблюдения с шагом
  150 мс (тест `TestQuarantineExitRequiresStableQuiescence`); первая
